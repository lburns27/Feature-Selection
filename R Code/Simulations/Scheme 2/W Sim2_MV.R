...
makeDataWMV2 <- function(N, small, large, tau, NoSig, lam1, lam2, alpha1, alpha2){
	Bo_small <- .133
	Bo_large <- .266
	Bt_small <- function(time) {Bo_small*log(lam1/lam2*alpha1/alpha2*time^(alpha1-alpha2))} 
	Bt_large <- function(time) {Bo_large*log(lam1/lam2*alpha1/alpha2*time^(alpha1-alpha2))} 
...
# 0% censoring
dat0 <- makeDataWMV2(N, numSmall, numLarge, "inf", notsig, 1,.5,3,2)

# 33% censoring
dat33 <- makeDataWMV2(N, numSmall, numLarge, 5.3, notsig, 1,.5,3,2)

# 67% censoring
dat67 <- makeDataWMV2(N, numSmall, numLarge, 2.4, notsig, 1,.5,3,2)

# 80% censoring
dat80 <- makeDataWMV2(N, numSmall, numLarge, 1.7, notsig, 1,.5,3,2)
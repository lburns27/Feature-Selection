...
makeDataLLMV <- function(N, small, large, tau, NoSig, lam1, lam2, alpha){
	Bo_small <- -.810
	Bo_large <- -1.621
	Bt_small <- function(time) {Bo_small*log(lam1/lam2*(1+lam2*time^alpha)/(1+lam1*time^alpha))} 
	Bt_large <- function(time) {Bo_large*log(lam1/lam2*(1+lam2*time^alpha)/(1+lam1*time^alpha))} 
...
# 0% censoring
dat0 <- makeDataLLMV(N, numSmall, numLarge, "inf", notsig, 2, 4, 2)

# 33% censoring
dat33 <- makeDataLLMV(N, numSmall, numLarge, 1.65, notsig, 2, 4, 2)

# 67% censoring
dat67 <- makeDataLLMV(N, numSmall, numLarge, 0.7, notsig, 2, 4, 2)

# 80% censoring
dat80 <- makeDataLLMV(N, numSmall, numLarge, 0.45, notsig, 2, 4, 2)